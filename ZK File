#!/bin/bash
# Generates the configuration for the ZooKeeper instance.

ZK_CONFIG_DIR=${ZK_CONFIG_DIR:-"/etc/zookeeper"}
ZK_CONFIG_FILE="$ZK_CONFIG_DIR/zookeeper.conf"
ZK_USER_GROUP=${ZK_USER_GROUP:-"root"}
...
LOGGER_PROPS_FILE="$ZK_CONFIG_DIR/log4j.properties"
JAVA_ENV_FILE="$ZK_CONFIG_DIR/java.env"
HOST=`hostname -s`
DOMAIN=`hostname -d`
...
function validate_env() {
    ...
    [ -z "$ZK_REPLICAS" ] && echo "ZK_REPLICAS is a mandatory environment variable" && exit 1
...
function create_config() {
    ...
    echo "#DO NOT EDIT ! This file was autogenerated" >> $ZK_CONFIG_FILE
    ...
    [ $ZK_REPLICAS -gt 1 ] && print_servers >> $ZK_CONFIG_FILE
...
function create_data_dirs() {
    ...
    if [ ! -d $ZK_DATA_DIR  ]; then
        mkdir -p $ZK_DATA_DIR
        chown -R :$ZK_USER_GROUP $ZK_DATA_DIR
    fi

    if [ ! -d $ZK_DATA_LOG_DIR  ]; then
        mkdir -p $ZK_DATA_LOG_DIR
        chown -R :$ZK_USER_GROUP $ZK_DATA_LOG_DIR
    fi

    if [ ! -d $ZK_LOG_DIR  ]; then
        mkdir -p $ZK_LOG_DIR
        chown -R :$ZK_USER_GROUP $ZK_LOG_DIR
    fi
    if [ ! -f $ID_FILE ]; then
        echo $MY_ID >> $ID_FILE
    fi
...
function create_log_props() {
    ...
    echo "zookeeper.root.logger=$ZK_LOG_LEVEL, CONSOLE" >> $LOGGER_PROPS_FILE
